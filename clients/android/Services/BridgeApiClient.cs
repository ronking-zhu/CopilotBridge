...
/// <summary>Talks to the Copilot bridge app server: /health, the async
/// /api/chat + /api/chat/{jobId} job API, and the /api/sessions CRUD. Always
/// sends the dev-tunnel anti-phishing bypass header and the optional X-API-Key.
/// Ported almost verbatim from the Windows client.</summary>
public class BridgeApiClient
{
    private static readonly JsonSerializerOptions JsonOpts = new() { PropertyNameCaseInsensitive = true };
    private readonly HttpClient _http;

    /// <summary>Default targets the Android emulator's alias for the host's localhost.</summary>
    public string BaseUrl { get; set; } = "http://10.0.2.2:3978";
