...
/// <c>FileSystem.AppDataDirectory/sessions/&lt;id&gt;.json</c>. The server is
/// authoritative for the session list; this cache only exists so the list and
...
internal static class LocalSessionCache
{
    ...
    private static string Dir => Path.Combine(FileSystem.AppDataDirectory, "sessions");

    /// <summary>Session ids come from the server (uuid); only allow filename-safe
    /// characters before touching the filesystem.</summary>
