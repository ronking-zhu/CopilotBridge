...
ones this bridge made). The bridge passes a session's id straight to Copilot as
``--session-id``, so a Copilot session id and a bridge session id share the same
uuid space - importing a Copilot session by id therefore lets the bridge *resume*
the real conversation on the next turn.
...
(tables ``sessions`` + ``turns``). Those are a different product from the CLI, so
the in-editor conversation only shows up once we read that DB too.
...
def _db_list_sessions(db_path: str) -> list[dict]:
    ...
    try:
        ...
        for (sid, summary, cwd, repo, branch, created, updated) in cur.execute(
            ...
            try:
                mc = cur.execute(
                    "select coalesce(sum((user_message is not null and user_message<>'')"
                    " + (assistant_response is not null and assistant_response<>'')),0)"
                    " from turns where session_id=?", (sid,)).fetchone()[0]
            except sqlite3.Error:
                mc = 0
            title = _clean_db_title(summary)
            if (not title) or str(summary or "").lstrip().startswith("<"):
                try:
                    fr = cur.execute(
            ...
            out.append({
                ...
                "repository": repo or "",
                "branch": branch or "",
                "createdAt": _to_epoch(created),
                "updatedAt": _to_epoch(updated) or _to_epoch(created),
                "messageCount": mc,
                "source": "vscode",
            })
...
def _db_read_session(db_path: str, sid: str) -> dict | None:
    ...
    try:
        ...
        for (_tidx, um, ar, ts) in cur.execute(
            ...
            t = _to_epoch(ts)
            if um and um.strip():
                messages.append({"role": "user", "text": um.strip(), "ts": t})
            if ar and ar.strip():
                messages.append({"role": "assistant", "text": ar.strip(), "ts": t})
        title = _clean_db_title(summary)
        if (not title) or str(summary or "").lstrip().startswith("<"):
            for m in messages:
                if m["role"] == "user":
...
def _count_messages(events_path: str) -> int:
    """Cheaply count user+assistant message events without full JSON parsing."""
    count = 0
    try:
...
def _summary_from_dir(entry_path: str, sid: str) -> dict | None:
    ...
    if updated is None:
        try:
            updated = os.path.getmtime(events_path if os.path.isfile(events_path) else ws_path)
        except OSError:
            updated = 0.0
    return {
        ...
        "createdAt": created,
        "updatedAt": updated,
        "messageCount": _count_messages(events_path),
        "source": "cli",
    }
...
def list_sessions(limit: int = 200) -> list[dict]:
    ...
    CLI session-state dirs (richest, with events.jsonl) take precedence; VS Code
    Copilot Chat sessions from its separate session-store.db are merged in and
    de-duplicated by id. No messages are included.
    """
    out: list[dict] = []
    seen: set[str] = set()
...
def read_session(session_id: str) -> dict | None:
    """Full transcript of a session by id: CLI session-state first, then the
    VS Code Copilot Chat database(s). Returns None if not found anywhere."""
